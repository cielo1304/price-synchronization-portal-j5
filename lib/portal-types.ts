...
export type Cell = {
  ...
  roMatch?: {
    ...
    key: string;
    /**
     * Внутренний product_id из РО (формат «14870175», в исходной таблице
     * пишется с пробелом «14870 175»). Не индексирован в `?search=`,
     * используется только как ключ для GET /products/{id}, чтобы получить
     * artikul. Заполняется из исходного JSON.
     */
    partProductId?: string | null;
    /**
     * Артикул товара в РО (формат «211149») — короткий номер, по которому
     * РО ищет в `?search=`. Если присутствует — используется напрямую,
     * иначе сервер резолвит его из partProductId через GET /products/{id}.
     */
